...
import { mediaIdSchema, FILE_SIZE_LIMITS, validateFilePath } from '../../utils/validation.js';
...
async function handleMediaUploadTool(args: unknown, apiClient: WechatApiClient): Promise<WechatToolResult> {
  const { action, type, filePath, fileData, fileName, mediaId, title, introduction } = mediaUploadToolSchema.parse(args);
  ...
  switch (action) {
    case 'upload': {
      ...
      if (fileData) {
        ...
        uploadData = await readFile(resolvedPath);
        uploadFileName = fileName || resolvedPath.split('/').pop() || 'media';
      }
      ...
      // 保存到本地存储
      const storageManager = apiClient.getAuthManager().getStorageManager();
